...
from ns_cifar10 import input_data
from ns_cifar10.utils import analyze_training_activations
from ns_cifar10.utils import cal_keep_probs_cdf
from ns_cifar10.utils import get_keep_probs


###====================Define parameters======================
selected_layers = ["conv1", "conv2", "pool3", "conv4", "conv5", "pool6", "fc7", "fc8"]
is_selected = {"conv1":False, "conv2":False, "pool3":False,
               "conv4":False, "conv5":True,  "pool6":True,
               "fc7"  :True,  "fc8"  :True}
num_neurons = {"conv1":30*30*64,  "conv2":28*28*64,  "pool3":14*14*64,
               "conv4":12*12*128, "conv5":10*10*128, "pool6":5*5*128,
               "fc7"  :256,       "fc8"  :256}
p_values    = {"conv1":1,         "conv2":1,         "pool3":1,
               "conv4":1,         "conv5":0.75,      "pool6":0.75,
               "fc7"  :0.7,       "fc8"  :0.95}
...
reader = tf.train.NewCheckpointReader("./runs/ckpt/cifar10_cnn_attacked.ckpt")

# CONV1: Convolution + ReLU.
# input: 32x32x3, filters: 3x3x64, output: 30x30x64.
w_conv1 = reader.get_tensor("Variable")
b_conv1 = reader.get_tensor("Variable_1")
h_conv1 = tf.nn.relu(conv2d(x, w_conv1) + b_conv1)

# CONV1 Selected.
h_conv1_flat = tf.reshape(h_conv1, [-1, 30*30*64])
h_conv1_flat_selected = select_wrapper(h_acts=h_conv1_flat,
                                       keep_probs=conv1_keep_probs,
                                       type="cos", k=1)
h_conv1_selected = tf.reshape(h_conv1_flat_selected, [-1, 30, 30, 64])

# CONV2: Convolution + ReLU.
# input: 30x30x64, filters: 3x3x64, output: 28x28x64.
...
h_conv2_flat_selected = select_wrapper(h_acts=h_conv2_flat,
                                       ...
                                       type="cos", k=1)
h_conv2_selected = tf.reshape(h_conv2_flat_selected, [-1, 28, 28, 64])

# POOL3: Max Pooling.
# input: 28x28x64, pooling: 2x2, output: 14x14x64.
...
h_pool3_flat_selected = select_wrapper(h_acts=h_pool3_flat,
                                       ...
                                       type="cos", k=1)
h_pool3_selected = tf.reshape(h_pool3_flat_selected, [-1, 14, 14, 64])

# CONV4: Convolution + ReLU.
# input: 14x14x64, filters: 3x3x128, output: 12x12x128.
...
# CONV4 Selected.
h_conv4_flat = tf.reshape(h_conv4, [-1, 12*12*128])
h_conv4_flat_selected = select_wrapper(h_acts=h_conv4_flat,
                                       keep_probs=conv4_keep_probs,
                                       type="cos", k=1)
h_conv4_selected = tf.reshape(h_conv4_flat_selected, [-1, 12, 12, 128])

# CONV5: Convolution + ReLU.
# input: 12x12x128, filters: 3x3x128, output: 10x10x128.
...
# CONV5 Selected.
h_conv5_flat = tf.reshape(h_conv5, [-1, 10*10*128])
h_conv5_flat_selected = select_wrapper(h_acts=h_conv5_flat,
                                       keep_probs=conv5_keep_probs,
                                       type="cos", k=1)
h_conv5_selected = tf.reshape(h_conv5_flat_selected, [-1, 10, 10, 128])

# POOL6: Max Pooling.
# input: 10x10x128, pool: 2x2, output: 5x5x128.
h_pool6 = max_pool_2x2(h_conv5_selected)
h_pool6_flat = tf.reshape(h_pool6, [-1, 5*5*128])
...
###=======Using a subset(about 100 batches) of CIFAR10 train data
###========to save the correct activations of the layers to be selected.
# traindata = input_data.read_cifar10_data_sets(data_dir="./data/cifar10", one_hot=True).train
# num_batches = 100
# is_selected = {"conv1":False,    "conv2":False,    "pool3":False,
...
if adv_method=="normal":
    testdata = input_data.read_cifar10_data_sets(data_dir="./data/cifar10", one_hot=True).test
else:
    testdata = input_data.read_adv_data_sets(advdata_dir="./data/cifar10/" + adv_method,
